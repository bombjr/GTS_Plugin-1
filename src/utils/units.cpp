...
namespace Gts {
	float unit_to_meter(const float& unit) {
		// Game reports that the height of a slaughterfish is 0.31861934f
		// From inspecting the bounding box of the slaughterfish and applying
		// base actor scales the unit height is 22.300568f
		// Assuming 0.31861934f is meters and that bouding box is in model unit space
		// then the conversion factor is 70
		// Slaughterfish was chosen because it has scales of 1.0f (and was in my worldspace)
