...
namespace Gts {
	...
	class Runtime : public EventListener {
		public:
			...
			static void Play_Sound(const std::string_view& tag, Actor* actor, const float& volume, const float& frequency);
			static void Play_Sound(const std::string_view& tag, TESObjectREFR* ref, const float& volume, const float& frequency);
